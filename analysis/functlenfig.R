...
filterDowners = function(.df.anl,thisL) { # to avoid using these lines twice... filters down data
  .d = .df.anl[.df.anl$wlen==thisL&.df.anl$k<=thisL & .df.anl$category=="function",]
...
pdf(file='fig5-RT_funct_by_k.pdf',width=8,height=5.5)
...
for (i in 1:length(lrange)) {
  ...
  if (i == 1) {
    plot(dat$Group.1,dat$x[,1],type=ltyp,lwd=lwd, # plot low-information items
         ylim=c(miny,maxy),xlim=c(-4,24),xaxt='none',
         pch=pchLow,col='darkgreen',
         xlab='',ylab='IKI (ms)',
         main='IKI for Functions Words by Length and Predictability')
  } else {
  ...
  text(textx,255,paste('k =',thisL),cex=.8,pos=4) # specify length on plot
  #error.bar(1:i,dat$x[,1],2*dat$x[,2])
}
...
text(-4,255,paste('\"the\"'),cex=.8,pos=4)
